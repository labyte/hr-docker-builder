# HR Docker 批量构建工具 — 技术方案与开发计划

> 📘 本文件为**技术方案与开发过程记录**（面向开发者/维护者）。软件使用说明请见 [README 使用手册](../README.md)。
>
...
> 待实施：构建历史持久化（F9）；manifest 多平台合并（明确 v1 不做）。
> 更新（2026-09-21）：M5 离线依赖包导出/导入已接入（工具栏「导出/导入离线包」，后端 `offline_pack.rs`）；M6 三平台安装包改由 GitHub Actions 自动构建发布（`v*` tag 触发，见 `.github/workflows/build.yml`），首个版本 v0.1.1。
...
## 10.5 离线包 v2：本地 registry mirror（2026-09-21 实施，v0.1.5）

交叉架构离线构建的 base 镜像问题（buildkit 独立存储不看本机 docker 库）以**本地 registry mirror** 方案解决：

- 导出：临时 `registry:2` 容器 + `docker buildx imagetools create` 以 CLI 客户端侧把每个 FROM 引用按**多架构 manifest** 复制进本地 registry；registry 卷数据 `docker cp` 出为 `registry-data/<i>/`；工具镜像与本机架构 base 走 `docker save/load` 双通道；
- 导入：`docker load` → 每个上游起一个 `hr-offline-reg-<i>` 常驻容器（`--restart unless-stopped`，Linux 绑 0.0.0.0、桌面绑 127.0.0.1）→ `docker cp` 灌数据 → 生成 `buildkitd.toml`（`[registry."<host>"] mirrors=["host.docker.internal:<port>"]` + http/insecure）→ 带 `--config` 重建 builder；
- 机制已在 macOS 宿主实测：buildkit 的 mirror 命中与 404→上游回退两条路径均有 registry 访问日志佐证；
- 已知环境怪癖：部分 Docker Desktop + 代理工具（TUN）组合下，imagetools 在**嵌套执行器 netns 内**push 会被内部代理劫持返回 502——导出务必保持 CLI 客户端侧执行（当前实现即是）；
- digest 固定引用（`FROM x@sha256:`）无法按 tag mirror，仅进同架构通道（导入时日志提示）。

## 11. 版本发布流程（维护者）

> 面向使用者的下载与安装说明在 [README 使用手册](../README.md)，本节仅记录发布操作。

1. **同步版本号**（三处 + 一处锁文件）：
   - `package.json`、`src-tauri/tauri.conf.json`、`src-tauri/Cargo.toml`
   - `cd src-tauri && cargo update -p hr-docker-builder --offline`（刷新 Cargo.lock）
2. **合入 main**：推送 `main` 会自动触发四平台构建（产物挂在 run 上供冒烟测试，不发版）；
3. **打 tag 发布**：`git tag vX.Y.Z && git push origin vX.Y.Z` → CI 构建 + 自动创建 GitHub Release。

CI 细节（`.github/workflows/build.yml`）：

- 矩阵：macOS aarch64（dmg）/ Linux x86_64（deb+AppImage）/ Linux aarch64（deb+AppImage，`ubuntu-24.04-arm` 原生构建，无交叉编译）/ Windows x64（msi）；
- **Release 仅由 `v*` tag 触发**，分支 push 时自动跳过；
- 产物与 Release 附件**只收最终安装包**（`.dmg/.msi/.deb/.AppImage`），过滤 Tauri `bundle/` 下的解包暂存目录——历史上曾因 `path: bundle/` 全量上传导致 Release 混入 261 个解包文件，勿回退；
- 网络提示：本机访问 GitHub 走代理（见 CLAUDE.md 规则）。
