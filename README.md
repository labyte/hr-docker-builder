...
- **齿轮菜单**：「全局设置」与「关于」——关于页展示当前版本号、简介、仓库与版本下载链接（可一键复制）。
...
设置面板（Docker Desktop 风格）：**左侧菜单四个分区、右侧对应表单**——「构建」（并发数、失败策略）/「Registry」（仓库前缀）/「命名」（Builder 名称、Tag 模板）/「数据」（数据目录）。每个分区独立保存，互不影响。工具条上的架构、产物去向随选随存。
...
1. **在线机**：项目配好后点工具条「导出离线包」，选择目标目录 → 生成 `offline-pack/` 文件夹：
